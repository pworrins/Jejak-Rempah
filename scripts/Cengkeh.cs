...
public partial class Cengkeh : Area3D
{
	// Signal untuk ketika cengkeh dikumpulkan dengan nilai poin
	[Signal]
	public delegate void CengkehCollectedEventHandler(int points);

	public CollisionShape3D col;
	public AudioStreamPlayer spice_fx;

	[Export]
	public float spin_speed = 4.0f;
	
	[Export]
	public int point_value = 5; // Total poin jika mengenai cengkeh = 5
	...
	public void Collected(Player body)
	{
		...
		EmitSignal("CengkehCollected", point_value);
		
		// Debug print untuk memastikan signal dipanggil
		GD.Print($"Cengkeh collected! Points: {point_value}");
