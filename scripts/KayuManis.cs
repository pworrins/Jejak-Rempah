...
public partial class KayuManis : Area3D
{
	// Signal untuk ketika kayu manis dikumpulkan dengan nilai poin
	[Signal]
	public delegate void KayuManisCollectedEventHandler(int points);

	public CollisionShape3D col;
	public AudioStreamPlayer spice_fx;

	[Export]
	public float spin_speed = 2.5f;
	
	[Export]
	public int point_value = 10; // Total poin jika mengenai kayu manis = 10
	...
	public void Collected(Player body)
	{
		...
		EmitSignal("KayuManisCollected", point_value);
		
		// Debug print untuk memastikan signal dipanggil
		GD.Print($"Kayu Manis collected! Points: {point_value}");
